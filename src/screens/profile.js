...
<div style="font-size:18px;font-weight:800;color:#fff;margin-bottom:8px">${gIcon} ${gLabel}</div>
<div style="display:flex;gap:16px;font-size:12px;color:rgba(255,255,255,.85)">
...
${rows.map(([k, v], i) => `<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;${i < rows.length - 1 ? 'border-bottom:1px solid ' + C.border : ''}">
<div style="font-size:14px;color:${C.ink}">${k}</div><div style="font-size:14px;color:${C.sub}">${v}</div></div>`).join('')}
</div>
